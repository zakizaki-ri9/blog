...
- ドキュメント、コメント、手順書に **絶対パス**（例: `/Users/...`）を記載しない
- パスは必ずプロジェクトルート（`blog/`）基準の **相対パス** で記載する
...
新機能の実装や大規模な変更を行う前に、**cc-sdd** を活用した仕様駆動開発を実施します。
...
- **ワークフローの概要** (`/sdd`):
  1. **Phase 0**: Steering（プロジェクト指針）の確認・更新
  2. **Phase 1: 仕様策定**
     - 要件定義（`requirements.md`）: EARS フォーマットで要件を明確化
     - 設計（`design.md`）: アーキテクチャ、コンポーネント、データモデル、API 仕様
     - タスク分解（`tasks.md`）: 実装タスクの洗い出しと依存関係の整理
  3. **Phase 2**: 実装計画書（`implementation_plan.md`）の作成とユーザー承認
  4. **Phase 3**: 承認後、EXECUTION モードで実装開始

- **仕様ファイルの配置**: `.kiro/specs/{feature-name}/`
- **詳細**: `.agent/skills/sdd/SKILL.md` および `.agent/workflows/sdd.md` を参照

- **既存ワークフローとの連携**:
  - SDD は `/review`, `/branch`, `/commit`, `/pr`, `/all-commit` の**前段階**として機能
  - 実装前の設計フェーズを充実させることで、後続の実装・レビュー・PR 作成を円滑化
