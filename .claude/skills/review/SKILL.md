...
**ドキュメント同期（今回のような仕様改修向け）**
- [ ] 実装変更に合わせて、関連ドキュメント（例: `docs/coding-guidelines/*.md`, `README.md`）が更新されているか？
- [ ] 新しい運用ルール・制約（例: `devOnly`, 命名規則, ディレクトリ構成, バリデーション方針）がドキュメントに反映されているか？
- [ ] ドキュメント変更が不要な場合、その理由がPR説明やレビューコメントに明記されているか？
