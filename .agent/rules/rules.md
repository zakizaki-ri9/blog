...
- 必ず計画 -> 承認待ち -> 実行のフローとする
- 承認が行われない限り、コード変更やパッケージのインストールといった実装は行ってはいけない
...
## 8. 提案と情報の信頼性 (Integrity & Evidence)

- **ソースの提示義務**: 技術的な提案や仕様の変更を提案する際は、必ずその根拠となる **一次情報（公式サイト、公式ドキュメント、信頼できる技術ブログ等）のURL** を提示してください。
- **推測の明示**: エージェント独自の推論や、ソースに基づかない提案を行う場合は、**「これは推測に基づく提案です」** と明記し、どこからどこまでが推測なのかを範囲指定してください。
- **リスク警告**: 確実なソースがない提案を行う場合は、**「ソースなき提案（独自提案）」** である旨を警告し、採用にはユーザーの慎重な判断が必要であることを伝えてください。
- **捏造の禁止**: URL、メールアドレス、コマンドオプションなどを「ありそうだから」という理由で想像して生成することは厳禁です。