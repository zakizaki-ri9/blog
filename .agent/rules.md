...
- **相対パスの徹底**: ドキュメントやアーティファクト内でプロジェクト内のファイルを参照する際は、**必ず相対パス** (`../docs/...` 等) または **リポジトリルート相対パス** (`/docs/...`) を使用してください。
- **禁止事項**: ユーザーのホームディレクトリを含む絶対パス (`file://...`) は、環境依存性が高いかつ、セキュリティリスクが高いため絶対に使用しないでください。
