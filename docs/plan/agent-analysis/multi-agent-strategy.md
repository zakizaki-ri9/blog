...
|---|---|---|---|---|
| **全体指示** | `claude.md` | `Gemini.md` | – | `copilot-instructions.md`（実装済み） |
| **クロスツール** | `AGENTS.md` | – | – | `AGENTS.md`（共有） |
| **ルール** | `.agent/rules/rules.md` | – | `.cursor/rules/coding.mdc` | 要実装 |
| **スキル/コマンド** | `.agent/skills/` | `.gemini/commands/` | `.cursor/commands/` | `.claude/skills/`（実装済み） |
...
[観点切り替え]
  ...
  GitHub Copilot : .claude/skills/*/SKILL.md (ドロップダウン選択)
  Gemini CLI     : コマンド駆動
...
✓ .claude/skills/*/SKILL.md              : .agent/skills/ の Copilot 版（同期）
...
| **Copilot** | `.md` + `.claude/skills/` | ドロップダウン選択 + 自動推論 | 明示的選択可、推論あり |
...
3. **実装時のチェックリスト**
  - `.agent/skills/` の Skill と `.claude/skills/` の整合性を維持
...
- [ ] `.claude/skills/` ディレクトリ実装
  - `.agent/skills/sdd/SKILL.md` → `sdd/SKILL.md` に移行
...
**A. 最小限対応（推奨: 短期）**
- Copilot: `copilot-instructions.md` + `.claude/skills/` のみ実装
